...
function showNotification(playerId, msg, color) {
  notifications.push({ playerId, msg, color, timer: 120 }); // 4초
...
function drawUI(p, phase, timeLeft, counts) {
  ...
  // ── 타임바 (남은 시간 시각화) ──
  // 상단에 얇은 타임바 추가
  const timeFraction = Math.max(0, timeLeft / GAME_TOTAL_TIME);
  p.fill(30);
  ...
  if (phase === PHASE_BETRAYAL) {
    ...
    p.textSize(15);
    p.text(`⚠ 배신 ${timeStr} ⚠`, CANVAS_W / 2, 16);
  } else if (phase === PHASE_SOLO) {
    p.fill('#FF9800');
    p.textSize(14);
    p.text(`⏱ 제한 ${timeStr}`, CANVAS_W / 2, 16);
  } else {
    p.fill(220);
    p.textSize(13);
    p.text(timeStr, CANVAS_W / 2, 16);
  }

  // ── 페이즈 라벨 ──
  p.textSize(9); p.textAlign(p.CENTER, p.BOTTOM);
  if (phase === PHASE_COOP)     { p.fill('#4CAF50'); p.text('[ 협력 페이즈 ]', CANVAS_W/2, 42); }
  else if (phase === PHASE_SOLO){ p.fill('#FF9800'); p.text('[ 한 명 사망 — 20초 제한! ]', CANVAS_W/2, 42); }
  else if (phase === PHASE_BETRAYAL){ p.fill('#FF5252'); p.text('[ 배신 페이즈 — 팀원도 적! ]', CANVAS_W/2, 42); }
  ...
  if (zombieBloodTimer > 0) {
    ...
    p.text(`🩸 좀비 가속 ${Math.ceil(zombieBloodTimer/FRAME_RATE)}초`, CANVAS_W/2, hudH + 4);
  }
...
function drawBetrayalAnnounce(p) {
  ...
  p.textSize(28); p.text('⚠ 배신 타이머 발동! ⚠', CANVAS_W/2, CANVAS_H/2 - 12);
...
function drawLobby(p) {
  ...
  const cx = CANVAS_W / 2, cy = CANVAS_H / 2;
  p.textAlign(p.CENTER, p.CENTER);
  p.textSize(38); p.fill('#4CAF50'); p.text('좀비 영역 전쟁', cx, cy - 150);
  p.textSize(14); p.fill(180); p.text('2인 협력 → 배신 영역 점령 게임', cx, cy - 105);
  p.textSize(13);
  p.fill(COLOR_A); p.text('플레이어 A: W A S D', cx - 120, cy - 55);
  p.fill(COLOR_B); p.text('플레이어 B: ↑ ↓ ← →', cx + 120, cy - 55);
  p.textSize(11); p.fill(160);
  p.text('협력 페이즈: 팀으로 좀비에 맞서 영역 확장', cx, cy - 15);
  p.text('배신 페이즈: 마지막 1분, 더 많은 영역 보유자 승리!', cx, cy + 8);
  p.fill(255, 165, 0);
  p.text('💊 약: 보너스 땅   🩸 피: 좀비 가속   ⚡ 에너지드링크: 속도2배+강철꼬리', cx, cy + 40);
  p.fill(180);
  p.text('좀비 꼬리를 밟으면 좀비가 죽습니다!', cx, cy + 62);
  const blink = Math.floor(p.frameCount / 20) % 2 === 0;
  p.fill(blink ? '#4CAF50' : '#2E7D32'); p.noStroke();
  p.rect(cx - 100, cy + 90, 200, 46, 10);
  p.fill(255); p.textSize(16);
  p.text('시작하기 (SPACE)', cx, cy + 114);
}
